  ...
  return (
    <section className="relative flex flex-col-reverse md:flex-row items-center justify-between min-h-[60vh] px-6 py-12 md:py-20 gap-8">
      <div className="flex-1 flex justify-center md:justify-start items-center order-2 md:order-1 mb-6 md:mb-0">
        <Avatar className="w-48 h-48 md:w-72 md:h-72 border-4 border-white/20 shadow-glass bg-card">
          <AvatarImage
          ...
          />
          <AvatarFallback className="text-4xl md:text-5xl font-bold bg-muted/70 text-white">
            SS
          </AvatarFallback>
        </Avatar>
      </div>
      <div className="flex-1 flex flex-col items-center md:items-start justify-center md:pl-10 order-1 md:order-2">
        ...
        <span className="block text-xl md:text-2xl font-semibold text-white mb-4 h-10 w-full text-center md:text-left">
          <TypingEffect
            words={TYPING_WORDS}
            typingDelay={220}
            erasingDelay={90}
        ...
        <div className="flex gap-4 mb-4">
          {SOCIALS.map((item) => (
            <a
              ...
              className="hover:scale-110 transition-transform p-2 rounded-full bg-card/70 border border-white text-white"
            >
              <item.icon size={20} />
            </a>
          ))}
